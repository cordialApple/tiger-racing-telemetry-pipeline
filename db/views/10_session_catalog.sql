...
SELECT s.session_id, s.source_file, s.platform, s.event, s.vehicle, s.racer,
       s.championship, s.session_name, s.comment, s.started_at,
       -- loggers record local wall clock, so labels must convert back or an evening run reads as next-day UTC
       s.started_at AT TIME ZONE 'America/Chicago' AS started_at_local,
       s.duration_s,
